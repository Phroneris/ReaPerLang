...
sub readTxt
{
	...
	# my @default = ('JPN_Phroneris', '580rc1', '5965');
	my $i = $_[0];
	if ($devmode==1)
	{
		$rname = '';
...
$indent = ' ';
print "\n\n";
print ' Files in & out (make sure that all are UTF-8 text files)', "\n";
...
print ' ┗━━━━━━━━━━━━━━━━━━━━━━━━', "\n";
print "\n";
print ' Enter your old LangPack name (without extension):', "\n", '  > ';
my @lng_old = &readTxt(0);
my @lng_dsc = &divDsc(1, @lng_old);
@lng_old = &divDsc(0, @lng_old);

print ' Enter the [version] of "template_reaper[version].*" ';
...
"┃in : ${rfiles[0]}",
"┃in : ${rfiles[2]}",
"┃out: _${mode}_lng_new.${ext[0]}",
"┃out: _${mode}_lng_missing.txt",
"┃out: _${mode}_tmp_crr.${ext[2]}",
"┃Keep empty sections: ${emp_section}",
"┃Date: ${date} (local)",
"┃Time: ${time} sec",
"┗━━━━━━━━━━━━━━━━━━━━━"
);
splice @header, 3, 0, "┃in : ${rfiles[1]}" if $mode == 1;
...
sub writeTxt
{
	my $name = shift;
	my $i = shift;
	my $e = defined $i ? $ext[$i] : 'txt';
	my $f;
	my $wname = "_${mode}_${name}.${e}";
	my @txt = eval "\@$name";
	map { $_=$_."\n" } @txt;
